...
async def sync_qiaofei(request: Request):
    try:
        # 1. Login to Qiaofei
        ...
        list_resp = session.post(list_url, params=params, json={"page": 1, "page_size": 50}, headers=headers)
        ...
        orders = list_data.get('data', {}).get('list', [])
        
        all_tickets = {}
        
        # 3. For each Order, fetch the Tickets
        for order in orders:
            ...
            ticket_resp = session.post(ticket_url, params=params, json={"cut_order_id": cut_order_id}, headers=headers)
